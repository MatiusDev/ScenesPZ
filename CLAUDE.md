...
### Writing the brief is the orchestrator's job, and a bad one costs more than the work

Measured on 2026-08-11: one `pz-lua` task spent **262,000 tokens and 63 tool calls** rewriting a
single 435-line file, and a `pz-review` of a **one-file** diff read eleven vanilla files, four
vendored files and five mod files. The user named it: *"siento que review analiza cada pieza del
código incluyendo archivos que ni se tocaron."*

Three causes, and two of them were mine:

1. **`pz-lua` had no Bash.** It could not run `lint.sh` or `audit.py` — it reported that as a
   blocker three sessions running — and it could not use `brain find` or `codegraph explore`,
   so it read whole files to find single facts. Fixed: it has Bash, and its definition now
   opens with the context-acquisition table.
2. **No agent knew the fast tools existed.** `.brain/`, `.codegraph/` and the `brain` CLI are
   all present in this repo and were mentioned in none of the five agent definitions.
3. **My briefs sent reviewers out of scope.** Asking "verify the cross-file seam" is sometimes
   right and always expensive. It is now an explicit, named exception rather than a habit.

So, when writing a brief:

- **Hand over every fact you already verified**, with its `file:line`, under a heading that says
  *do not re-derive these*. A fact you paid for once should never be paid for twice.
- **Name the files the agent owns, and the files it must not write.** Parallel writers that
  collide cost a whole task.
- **Point at the two vendor documents instead of letting an agent open a 2,493-line file.**
  `docs/BANDITS-API.md` is what the framework MEANS; `docs/VENDOR-INDEX.md` is WHERE everything
  is — the line span of every function in the loaded version, generated by
  `./tools/vendorindex.py`, meant to be **grepped, never read**. Regenerate it after
  `./tools/deps.py update`. Keep BANDITS-API fed: when a review reports re-deriving something,
  add it there the same session.
- **Scope the review to the diff.** Only name an out-of-diff file when a specific seam needs
  checking, and say why. Absent that instruction, out-of-diff is out of scope.
...
- `docs/VENDOR-INDEX.md` — **grep it, never read it.** Generated by `./tools/vendorindex.py`:
  the line span of all 2,232 functions in the vendored version the game actually loads.
  `grep -n '`ManageCombat`' docs/VENDOR-INDEX.md` → `898-1469`, then read those lines instead
  of the 2,493-line file. It also indexes `Foo.Bar = function(...)` declarations, which are
  invisible to `grep "function Foo.Bar"` and have hidden two libraries from us. Scoped to the
  loaded version on purpose — `vendor/` ships 42.12 through 42.20 side by side plus a
  `Bandits.old` tree, and indexing all of them produced a 948 KB file that cost more to read
  than the problem it solved. Regenerate after `./tools/deps.py update`.
